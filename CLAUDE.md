...
## 현재 버전
**v6.18** (2026-09-07) — 작업 브랜치

## 버전 히스토리 요약
| 버전 | 주요 변경 |
|------|-----------|
| v6.18 | **모바일 저장공간 부족 자동 백업 오류 수정** — 자동 백업 본문을 용량이 작은 localStorage가 아니라 IndexedDB 안전 저장소에 검증 후 보관하고, 기존 localStorage 백업도 검증 이전한 뒤 원본 키를 비워 공간 확보. IndexedDB를 쓸 수 없는 환경에서는 기존 localStorage 저장으로 안전하게 폴백. 같은 오류 토스트가 동시에 중복 표시되지 않도록 활성 알림 중복 제거. |
| v6.17 | **기록보관함 잠금 비밀번호 1회 설정 고정** — 이미 잠긴 기록/Dropbox 동기화본에 비밀번호 해시·salt가 있으면 이를 보관함 전체 PIN으로 승격해 새 잠금 때 다시 설정창을 띄우지 않음. 최초 1회 설정 후 이후 잠금은 같은 PIN을 재사용. |
| v6.16 | **상세 이미지 한턱 배지 간소화** — Dropbox 상세 이미지의 한턱 표기를 `🎉 한턱 쏨 🎉`으로 정리. |
| v6.15 | **정산 이미지 전용 폴더 + 상세 이미지 한턱 표시** — 신규 정산 이미지는 Dropbox `/정산내역`에 저장. 데이터 메뉴의 `정산 이미지 폴더 정리`로 루트의 기존 정산 PNG만 확인 후 이동하며, JSON·영수증·상세내역은 제외하고 이름 충돌 시 자동 이름으로 양쪽 파일을 보존. 상세 이미지 매장 카드에도 한턱 배지를 표시. |
| v5.96 | **데이터 보존·접근성 복구** — 화면에서 빠져 저장된 그룹을 전환할 수 없던 그룹 선택기를 데스크톱·모바일에 복원하고, 250개 이상에서도 찾을 수 있게 날짜·모임·참여자 검색과 최신순 정렬 추가. 같은 날짜·참여자만으로 다른 기록을 덮어쓰던 soft 병합 제거, 기록 갱신 시 상세 품목·사진 보존. 미업로드/미삭제 사진 키 영구 큐, 참조 없는 로컬·Dropbox 원본 정리, ID 기준 기록 중복 제거, Dropbox rev 조건부 저장, 인라인 이벤트 문자열 안전 인코딩 포함. |
...
| v5.90 | 수금 인원 목록 **한턱/참석 배지 오른쪽 끝을 상태 글자에 정렬**(사용자 요청) — 상태 값(초과/완납/미수)은 36px 셀 가운데정렬이라 글자 오른쪽 끝이 셀 오른쪽보다 7px 안쪽인데, 배지(`.pm-mrow-badge`)는 셀 오른쪽 끝(justify-self:end)에 붙어 상태 글자보다 7px 더 나가 있었음. `margin-right:7px`로 배지를 당겨 상태 글자 오른쪽 끝과 정확히 일치(헤드리스 실측 모바일·데스크탑 gap 0). 상태값은 항상 2글자라 7px 고정 |
| v5.89 | 수금 인원 목록 **구분선 불일치 수정**(사용자 요청, "머리글은 짧고 행은 길고 여분이 없다") — 원인은 행에만 걸린 음수 마진 `.pm-name-tap{margin:0 -6px}`가 행 구분선만 카드 밖으로 삐져나가게 한 것. `.pm-mtable .pm-month-person-row`에 `margin:0` 강제 → 머리글·모든 행이 상세영역 14px 패딩 안에 동일 정렬(구분선 좌우 여분 균일, 글자는 그 안쪽). 상태 값(초과/완납) 가운데정렬 유지. CLAUDE.md 표 구분선 규칙을 "좌우 끝까지"→"머리글·행 동일 14px 인셋"으로 정정 |
| v5.88 | 수금 인원 목록 폴리시(사용자 요청) — (1) **횟수는 이름 바로 옆(우측)**: v5.87의 인라인 위치 유지(이름과 붙어 '조상현 1회'), 머리글은 `참여자 · 횟수` 한 라벨로 이름 열 위에 좌측정렬. 독립 가운데 열로 멀리 뺐던 시안은 사용자가 "횟수는 이름 가까이 우측에"라 해 되돌림. (2) **상태**는 머리글·값 모두 가운데 정렬. (3) 차액·청구금액은 원 기준 우측정렬(유지). 그리드 4열 `minmax(0,1fr) 58/90/36`, 머리글 `참여자·횟수(좌) \| 차액(우) \| 청구금액(우) \| 상태(center)`. 한턱/참석 배지는 차액~상태 3열(2/-1)에 걸쳐 우측정렬. (4) **송금 색상 구분선(파랑/주황) 절반 두께**(`.txn-group-div` 3px→1.5px, radius 1px). (5) 인원 행 구분선은 v5.79대로 폭 꽉 찬 1px 직선·마지막 행 없음(균일) |
| v5.87 | 수금 월별 인원 목록에 **열 머리글 재도입**(사용자 요청) — `참여자 · 횟수 \| 차액 \| 청구금액 \| 상태`(.pm-mhead). v5.83에서 균일간격 위해 flex 클러스터로 바꾸며 머리글을 뺐던 걸, **고정 폭 열(minmax(0,1fr) 62/96/34) 그리드 + 화면 폭별 gap 조절**(모바일 10px·PC 22px)로 되돌려 머리글 정렬과 답답함 완화를 동시에 잡음. 결제 서브라인·14px 인셋·내 행 횟수는 유지. 참석/한턱 행 배지는 오른쪽 3열에 걸쳐 우측정렬 |
| v5.86 | **계산 안전장치 2겹**(사용자 요청 '계산 절대 실수 없도록') — (1) **런타임 트립와이어** `_settlementSanity(paid,owes,txns)`: 정산 렌더마다 강한 불변식(총 결제액===총 분담액)과 인당 순액(보낸−받은 vs 분담−결제, 반올림 오차 내) 점검, 어긋나면 콘솔 경고 + `window._settleWarn`에 기록(UI 무해). renderResultView에서 호출. (2) **test.html 대폭 확장 50→80건**: 깍두기(watchers) 제외(TC10c), 자기점검 배터리(TC10d — 균등·나머지·부분참여·다중결제·환불상계·한턱혼합·깍두기·차액지정·7인복합 9시나리오 × 총결제=총분담·자기점검 통과·수지 균형). computeShares/calcTxns 검토 결과 로직(float 누적→floor→최대잔여, 쌍별 부호순액 상계) 정상 확인 |
| v5.85 | **계산 버그 수정(중요)**: 정산 결과 '한턱 혜택 (X 쏨)' 행이 라벨은 첫 한턱 결제자인데 금액은 전원 한턱 합을 표시(예: 조상현 쏨인데 신유철 쏜 26,800원까지 더해 58,300원). `_resultBreakdownHtml`의 treatTotal을 `isTreat&&payer===ownerName`으로 좁혀 소유자가 쏜 금액만(31,500) 합산. **test.html에 회귀 TC10b 추가**(소유자 한턱만 합산 검증) — 전체 50건 통과. 예방책으로 '계산 변경 시 test.html 필수' 규칙 명문화. 아울러 앱 내 뒤로가기를 클릭 추측→**실제 상태(DOM) 관찰(MutationObserver) 기반 1:1 히스토리 모델**로 재작성해 '이랬다 저랬다'(탭 추적 어긋남) 근본 해결(popstate 중 pushState 금지 유지) |
| v5.84 | 앱 내 뒤로가기 '홈으로 갔다가 이전 화면으로' 이중 전환(안드로이드 깜빡임) 버그 수정 — 원인은 popstate 핸들러 안에서 다시 pushState(re-arm)한 것. 한 단계(탭 변경·오버레이·검색)마다 그 시점에 history 항목 1개를 push하고 back에서는 스택을 1개 pop해 그 단계만 되돌리는 1:1 모델로 재작성. **popstate 안에서는 절대 pushState 안 함**(stack/detect/pushEntry). 규칙: CLAUDE.md 뒤로가기 항목에 명시 |
| v5.83 | 수금 인원 목록/검색 통계창 폴리시(사용자 피드백 4종) + 레이아웃 규칙 명문화 — (1) **콘텐츠 인셋 복구**: v5.82에서 행 패딩을 4px로 줄여 글자가 구분선 가장자리로 나가던 문제를, 좌우 14px 인셋으로 복구(구분선 안쪽 여백). CLAUDE.md에 '리스트 콘텐츠 인셋' 규칙 명문화. (2) **오른쪽 값 균일 간격**: 고정 폭 열(자릿수에 따라 간격 변동)→우측정렬 flex 클러스터(gap 8px 균일)로 전환, 열 머리글 제거. '청구' 단어 없이 `+269원 288,026원 초과`. (3) **내 행 횟수 표시**(mySettleCount회). (4) **이름 검색 요약 카드 일관성**: 옛 pm-person-stats/pm-pstat(총/8월 청구·내 몫·한턱)를 월별과 동일한 pm-month-stats-row(청구·수령·미수 + 결제·지출·쏜, 미수 회색)로 통일. (5) **뒤로가기**에 이름 검색 해제 단계 추가(오버레이→검색해제→직전 탭) |
| v5.82 | 수금 월별 인원 목록을 **열 머리글 정렬 테이블(B안)**로 전환(사용자 선택) — '결제 X원' 배지가 이름 옆에서 폭에 밀려 두 줄로 깨지던 문제 해결. (1) 목록 상단에 머리글 `성명·차액·청구·여부`(.pm-mhead), 머리글과 모든 행이 동일 **고정 폭 열**(minmax(0,1fr) 62/92/34, gap10)을 써서 보이지 않는 세로선에 우측 정렬. (2) 행에서 '청구' 단어 제거 → `+174원 288,026원 초과` 형태. (3) **결제는 이름 아래 서브라인**(pm-mrow-paid)이라 줄바꿈 없음·없는 사람은 미표시. (4) 한턱/참석 행은 배지를 오른쪽에 걸쳐 우측정렬(pm-mrow-participant). _pmMonthlyHtml 마크업을 grid 셀 구조로 교체(pm-mrow-*), 결제·쏜 탭 동작 유지 |
| v5.81 | 수금 통계 카드(결제·지출·쏜 / 청구·수령·미수) 폴리시 3종(사용자 요청) — (1) 셀 사이 세로 구분선을 border-right에서 `::after` 인셋 선으로 바꿔 진하게(--sep2)+상하 여백(셀 패딩만큼 인셋 top14/bottom13 → 글자 높이만큼만). (2) 금액 글자 15px→12.5px로 2~3pt 축소. (3) 미수 '없음' 표기를 검정(--label)→회색(--label3, .mut). 인원 행 리디자인(열 머리글)은 시안 확정 후 별도 |
...
- **계산은 생명 — 회귀 테스트 필수:** 정산·분담·한턱·수금·수취 등 **금액을 계산·집계·표시하는 코드를 건드리면 반드시 `test.html`에 그 케이스를 추가하고 전체 통과를 확인한 뒤 배포한다.** test.html은 index.html을 iframe으로 불러 실제 함수를 호출하므로(computeShares·calcTxns·_resultBreakdownHtml 등) 로직 복사본이 갈라지지 않는다. 특히 **라벨과 금액이 반드시 일치**해야 한다 — 'X 쏨'이면 금액도 X가 쏜 것만(집계 대상을 `payer===그 사람`으로 좁혔는지 확인). 헤드리스 실행: 리포 루트를 http로 띄우고 test.html 로드해 PASS/FAIL 확인.

## 레이아웃/디자인 규칙 (반드시 지킬 것)
- **리스트 콘텐츠 인셋:** 리스트 행의 글자·값은 항상 구분선(행 폭 전체 직선) **안쪽**으로 들어가야 하며, 좌우에 여백(가로 ~14px)을 둔다. 콘텐츠가 카드/구분선 가장자리에 붙지 않게 한다. (수금 인원 목록 `.pm-mtable .pm-month-person-row{padding:9px 14px}` 참고)
- **구분선(중요·표 공통 규칙):** 행 사이 구분선은 1px 직선, **머리글·모든 행이 완전히 동일한 좌우 인셋(약 14px 여분)** 으로 정렬한다. 글자는 그 구분선 안쪽(패딩 14px 더). 한 표 안에서 머리글만 짧고 행만 카드 끝까지 나가는 식의 불일치 금지 — 원인은 대개 행에만 걸린 음수 마진(`.pm-name-tap{margin:0 -6px}` 등)이므로 `.pm-mtable` 문맥에서 `margin:0` 으로 눌러 머리글과 같은 폭으로 맞춘다. 마지막 행은 구분선 없음.
- **인원 목록은 열 머리글 정렬 테이블:** 머리글 `참여자 · 횟수 | 차액 | 청구금액 | 상태`(`.pm-mhead`)를 목록 상단에 두고, **고정 폭 4열 그리드**(minmax(0,1fr) 58/90/36)로 머리글과 모든 행이 같은 세로선에 정렬한다. 정렬 방향: 참여자·횟수=좌(**회 숫자는 이름 바로 옆 인라인**, 이름과 붙어 표시), 차액·청구금액=우측(원 기준), **상태=가운데**. 값 사이 간격은 **화면 폭에 따라 gap만 조절**(모바일 8px, PC 20px). 결제는 이름 아래 서브라인, 한턱/참석 배지는 차액~상태 3열(grid-column 2/-1)에 걸쳐 우측정렬. **횟수를 독립 가운데 열로 멀리 빼지 말 것**(사용자: "횟수는 이름 가까이 우측에").
- **통계 카드 일관성:** 월별 통계·이름 검색 요약 등 통계 표시는 동일 규격(`pm-month-stats-row`/`pm-mstat`, 라벨 청구·수령·미수 / 결제·지출·쏜, 금액 12.5px, 셀 세로 구분선 `--sep2` 글자 높이 인셋, 미수 '없음'은 회색 `.mut`)을 재사용한다.
- **소유자 행 횟수:** 내 이름(_myName) 행에도 다른 인원과 동일하게 '회'(정산 참여 횟수)를 표시한다.
- **앱 내 뒤로가기:** 브라우저·안드로이드 back은 앱을 벗어나지 않고 이전 단계로(오버레이 닫기 → 이름 검색 해제 → 직전 탭). 한 단계마다 그 시점에 `history.pushState` 1개를 쌓고(클릭/입력 캡처의 `detect`), back에서는 stack을 1개 pop해 그 단계만 되돌린다(1:1). **popstate 핸들러 안에서는 절대 pushState 하지 않는다** — 안드로이드에서 '홈으로 갔다가 이전 화면으로' 이중 전환(깜빡임)이 생긴다(v5.84).
