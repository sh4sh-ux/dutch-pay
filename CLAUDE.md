...
- **오른쪽 값 클러스터 간격 균일:** 한 행 오른쪽에 여러 값(예: 차액·청구·여부)을 나열할 때는 우측 정렬 flex 클러스터로 묶고 `gap`을 균일하게 준다(값끼리 간격이 서로 같아야 함). 고정 폭 열 우측정렬은 금액 자릿수에 따라 값 사이 간격이 달라지므로 지양.
