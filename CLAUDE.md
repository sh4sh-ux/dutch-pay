...
## 현재 버전
**v5.50** (2026-09-02) — 작업 브랜치

## 버전 히스토리 요약
| 버전 | 주요 변경 |
|------|-----------|
| v5.50 | 카테고리 선택창 아이콘을 영수증 DB와 동일 크기로 확대(데스크탑 24→27px, 모바일 20→22px). SVG는 벡터라 원본 그대로 두고 CSS만 수정. 헤더 padding을 실측 정렬 — '카테고리 선택' 왼쪽=1행 첫 아이콘 왼쪽, '자동' 글자 오른쪽=1행 끝 아이콘 오른쪽(오차 0.1px) |
