...
- **인원 목록은 열 머리글 정렬 테이블:** 머리글 `참여자 · 횟수 | 차액 | 청구금액 | 상태`(`.pm-mhead`)를 목록 상단에 두고, **고정 폭 열**(minmax(0,1fr) 62/96/34)로 머리글과 모든 행이 같은 세로선에 우측 정렬되게 한다. 값 사이 간격은 **화면 폭에 따라 gap만 조절**(모바일 column-gap 10px, PC 22px) — 열 정렬(머리글)과 답답함 완화를 동시에. 결제는 이름 아래 서브라인.
