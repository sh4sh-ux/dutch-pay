...
## 현재 버전
**v1.26** (2026-06-03) — 커밋 109cb27, main 브랜치 푸시 완료

## 이미지 생성 주요 상수 (generateSettleCanvas) — v1.22 기준
- TOTAL_CARD_H=245 (총 지출금액 카드)
- 금액 Y: y+81 (카드 높이의 33%, UI와 동일 비율)
- 구분선 Y: y+147
- 인당평균 Y: y+198
- ITEM_ROW_H=152 (지출 항목 행)

## 버전 히스토리 요약
| 버전 | 주요 변경 |
|------|-----------|
| v1.26 | 기록보관함 getElementById→querySelectorAll 전면 교체 (Android 카드/iOS 패널 근본 수정) |
| v1.25 | Android 카드 탭 버그 수정 시도, iOS Dropbox 패널 상태 동기화 |
| v1.24 | 기록 보관함 '품목'→'지출 내역' 통일, 아이콘 추가 |
| v1.23 | 이미지 폰트 스택 UI CSS와 일치 (SF Pro Text, Apple SD Gothic Neo) |
| v1.22 | 이미지 총액 카드 높이 245, 금액·구분선·인당평균 위치 균형 조정 |
| v1.21 | Dropbox 경로 한글→ASCII 수정 (Dutch-Pay 폴더) |
| v1.20 | Dropbox 저장 경로 변경 → /01_Personal/Dutch-Pay/ |
| v1.19 | 총 지출금액 카드 원 타이포그래피 통일, 이미지 구분선·인당평균 위치 개선 |
...
- 이미지 총액 카드: 구분선 y+130, 인당평균 y+178 (균등 여백)
- settle-hero "원": sh-unit 제거 — sh-num(34px 700) 인라인 동일 스타일 (v1.19~)
...
<div class="settle-hero">
  <div class="sh-num">117,900원</div>  ← 동일 34px 700, sh-unit 없음
